...
class HotelServices{
    async addHotel(data){
        const response = await NetworkServices.post(`${Config.SERVER_URL}/addHotel`,data);
    ...
    async getHotelById(id){
        const response = await NetworkServices.get(`${Config.SERVER_URL}/hotels/${id}`);
        return response;
    }

    async searchHotel(search){
        const response = await NetworkServices.get(`${Config.SERVER_URL}/hotels/search/${search}`);
        return response;
    }
}
